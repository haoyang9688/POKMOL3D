...
def vina_ligprep_process(config):
    ...
    config_redocking_output_path = config['settings']['Redocking_settings']['prepared_ligands_path']
    output_base_folder = os.path.join(config_redocking_output_path)  
    ...
    with Pool(processes=100) as pool:
        pool.starmap(process_folder, [(folder, output_base_folder) for folder in folders])

    #print("Ligprep processing completed")
