...
def process_folder(digit_folder_index, input_path, output_path):
    try:
        rmsd_results_main_folder = os.path.join(output_path, "Target-binding-metrics", "Vina-RDKit-RMSD-results")
                ...
                for reference_file in os.listdir(reference_folder):
                    if reference_file.endswith(".sdf"):
                        if os.path.splitext(input_file)[0] == os.path.splitext(reference_file)[0]:
                            output_file = os.path.join(rmsd_results_folder, f"rmsd-results-{input_file}-{reference_file}.txt")
...
def calculate_rmsd(config):
    try:
        ...
        for digit_folder in digit_folders:
            input_folder = os.path.join(output_path, "Target-binding-metrics", "Vina-Docking-results", str(digit_folder))
