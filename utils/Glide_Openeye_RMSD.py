...
def process_folder(digit_folder_index, input_path, output_path, rmsd_script_path):
    rmsd_results_main_folder = os.path.join(output_path, "Target-binding-metrics", "Glide-Openeye-RMSD-results")
    ...
    for input_file in os.listdir(input_folder):
        if input_file.endswith("_lib.sdf"):
            #Get the base file name of the input file (remove the suffix "- lib")
            input_file_base = os.path.splitext(input_file)[0].replace("_lib", "")
            input_file_path = os.path.join(input_folder, input_file)
            
            #Define the path of the reference file
...
def calculate_openeye_rmsd(config):
    ...
    existing_folders = [
        ...
        if os.path.exists(os.path.join(output_path, "Target-binding-metrics", "Glide-Docking-results", str(digit_folder))) and
